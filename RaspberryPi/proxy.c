...
int readGeonet(int clientSocket, SSL_CTX * sslCtx);
int writeGeonet(int clientSocket, SSL_CTX * sslCtx);
...
int main(int argc, char **argv)
{
    ...
    //On attend l'arrivée d'un client
...
int open_socket()
{
    ...
    //On s'assure qu'il n'y ai pas d'erreur
    if (s < 0) 
    {
...
int writeGeonet(int clientSocket, SSL_CTX * sslCtx)
{
    ...
    while(1)
    {
        ...
        fprintf(stderr, "Reading from app");
        ...
        fprintf(stderr, "Reading from app finished");
...
int readGeonet(int cleintSocket, SSL_CTX * sslCtx)
{
    //Buffer Ethernet
    char buffer[65536];
    ...
    while(1)
    {
        ...
        else 
        {
            ...
            if(eth->h_proto == 18313 && !(eth->h_source[0] == 0x00 && eth->h_source[1] == 0xff && eth->h_source[2] == 0x00 && eth->h_source[3] == 0x00 && eth->h_source[4] == 0x00 && eth->h_source[5] == 0x00))
            {
                ...
                {
                    fprintf(stderr, "SSL_accepted\n");
                    if(SSL_write(ssl, data, remaining_data)<0)
                    {
                        perror("Write failed\n");
                    }
                    else
