...
void CamApplication::on_timer(Clock::time_point)
{
    ...
    auto time_now = duration_cast<milliseconds>(runtime_.now().time_since_epoch());
    ...
    vanetza::asn1::Denm denmMessage;

    ItsPduHeader_t& dDeader = denmMessage->header;
    dDeader.protocolVersion = 1;
    dDeader.messageID = ItsPduHeader__messageID_denm;
    dDeader.stationID = 1; // some dummy value

    DecentralizedEnvironmentalNotificationMessage_t& denm = denmMessage->denm;

    position = positioning_.position_fix();
    ...
    copy(position, management.eventPosition);

    time_now = duration_cast<milliseconds>(runtime_.now().time_since_epoch());

    asn_long2INTEGER(&denmMessage->denm.management.detectionTime, time_now.count());
    asn_long2INTEGER(&denmMessage->denm.management.referenceTime, TimestampIts_utcStartOf2004);
    ...
    DownPacketPtr denmPacket { new DownPacket() };
    denmPacket->layer(OsiLayer::Application) = std::move(denmMessage);

    request.its_aid = aid::DEN;
    request.transport_type = geonet::TransportType::SHB;
    request.communication_profile = geonet::CommunicationProfile::ITS_G5;

    confirm = Application::request(request, std::move(denmPacket));
    if (!confirm.accepted()) {
        throw std::runtime_error("CAM application data request failed");
    }
}